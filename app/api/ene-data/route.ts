...
export async function GET(){
  ...
  try{
    ...
    const signature=JSON.stringify(metadata.map(({key,lastModified,etag,size})=>({key,lastModified,etag,size})));
    ...
    const payload=parseEneOfficialFiles({indicators:files.indicators,branches:files.branches,absent:files.absent});
