...
export async function GET(){
  ...
  try{
    ...
    const signature=JSON.stringify(metadata.map(({key,lastModified,etag,size})=>({key,lastModified,etag,size}))),now=new Date().toISOString();
    if(cached?.source_last_modified===signature){await db.prepare("UPDATE economic_source_cache SET checked_at = ? WHERE kind = ?").bind(now,"informality").run();return NextResponse.json({...JSON.parse(cached.payload_json),sources:metadata,cache:{status:"shared",checkedAt:now,updatedAt:cached.updated_at}},{headers:{"Cache-Control":"no-store"}});}
